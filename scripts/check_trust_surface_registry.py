...
def check_lean431_string_kernel_facts(root: Path = ROOT) -> list[str]:
    """Keep the Lean 4.31 string facts kernel-proved and in one module."""
    errors: list[str] = []
    boundary = root / LEAN431_STRING_FACTS
    if not boundary.is_file():
        return [f"missing Lean 4.31 string fact module: {LEAN431_STRING_FACTS}"]
    ...
    if native_count != 0:
        errors.append(
            f"{LEAN431_STRING_FACTS}: expected kernel proofs with 0 native_decide, "
    ...
    expected_path = str(LEAN431_STRING_FACTS)
...
def main() -> int:
    ...
    errors = check_lean431_string_kernel_facts()
