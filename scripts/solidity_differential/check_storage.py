...
from .programs import stateful_storage_source, stateful_storage_word_source
...
def main():
    ...
    parser.add_argument('--fixture', choices=('packed', 'void', 'bytes'), default='packed')
    ...
    name = {'packed': 'StorageSequence', 'void': 'StorageVoidSequence', 'bytes': 'StorageBytesSequence'}[args.fixture]
    ...
    completed = []
    baseline = None
    variants = ('baseline', 'bindings', 'reordered' if args.fixture == 'packed' else 'expression')
    for variant in variants:
        directory = output / variant
        directory.mkdir()
        source = directory / 'Sequence.sol'
        source.write_text(stateful_storage_source(fixture, variant) if args.fixture == "packed"
