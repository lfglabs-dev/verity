...
SPEC = {'check_only_paths': ['.github/workflows/**',
 ...
 'expected_jobs': ['closed-pr-cleanup',
                   ...
                   'foundry-multi-seed',
                   'failure-hints'],
 'heavy_runner_jobs': ['build',
                       'compiler-regressions',
                       'lean-profile'],
 'medium_runner_jobs': ['build-audits',
                        'build-compiler-binaries',
                        'compiler-audits',
                        'foundry-gas-calibration',
                        'foundry',
 ...
 'expected_job_if_conditions': {'closed-pr-cleanup': "github.event_name == 'pull_request' && "
                                ...
                                'failure-hints': "always() && github.event_name == 'pull_request' "
                                                 ...
                                                 "'failure')"},
 'expected_job_runs_on': {'closed-pr-cleanup': '[self-hosted, linux, verity, fastlane]',
                          'changes': '[self-hosted, linux, verity, fastlane]',
                          'checks': '[self-hosted, linux, verity, fastlane]',
                          'timeout-watchdog': '[self-hosted, linux, verity, fastlane]',
                          ...
                          'failure-hints': '[self-hosted, linux, verity, fastlane]'},
 'expected_job_timeouts': {'closed-pr-cleanup': 5,
                           'changes': 5,
                           'checks': 45,
                           'timeout-watchdog': 5,
                           'build': "${{ fromJSON((github.event_name == 'schedule' || (github.event_name == 'workflow_dispatch' && inputs.clean_build)) && '60' || '35') }}",
                           'build-audits': 20,
                           'build-compiler-binaries': 360,
                           'compiler-audits': 45,
                           'compiler-regressions': "${{ fromJSON((github.event_name == 'schedule' || (github.event_name == 'workflow_dispatch' && inputs.clean_build)) && '150' || '90') }}",
                           'lean-profile': 45,
 ...
 'expected_workflow_concurrency': {'group': "${{ github.workflow }}-${{ github.event_name == 'pull_request' && github.event.action == 'closed' && format('closed-pr-cleanup-{0}-{1}', github.event.pull_request.number, github.run_id) || (github.event_name == 'pull_request' && format('pr-{0}', github.event.pull_request.number) || (github.event_name == 'schedule' && format('schedule-{0}', github.ref) || github.ref)) }}",
                                   'cancel-in-progress': "${{ github.event_name != 'schedule' && !(github.event_name == 'pull_request' && github.event.action == 'closed') }}"},
 'expected_workflow_env': {'SOLC_VERSION': '0.8.33',
                           ...
                           'VERIFY_CLEAN_BUILD': "${{ (github.event_name == 'schedule' || (github.event_name == 'workflow_dispatch' && inputs.clean_build)) && 'true' || 'false' }}",
                           'VERIFY_FORCE_FULL_RUN': "${{ (github.event_name == 'schedule' || (github.event_name == 'workflow_dispatch' && inputs.force_full_run)) && 'true' || 'false' }}",
                           'VERIFY_USE_STICKY_DISKS': "${{ (github.event_name == 'schedule' || (github.event_name == 'workflow_dispatch' && inputs.clean_build)) && 'false' || 'true' }}",
                           'VERIFY_DISABLE_LAKE_CACHE_RESTORE': "${{ (github.event_name == 'schedule' || (github.event_name == 'workflow_dispatch' && inputs.clean_build)) && 'true' || 'false' }}",
 ...
 'expected_step_contracts': {'changes': [{'uses': 'actions/checkout@v6'},
                                              ...
                                              {'name': 'Setup Lean',
                                               ...
                                               'with': {'cache-key-prefix': 'lake',
                                                        ...
                                                        'cache-primary-key': 'lake-${{ runner.os '
                                                                             ...
                                                                             '}}-${{ github.run_id '
                                                                             '}}'}},
                                              {'name': 'Restore prepared Lean workspace build (local)',
                                               'run': 'scripts/ci_local_persistence.sh fetch \\\n  --run-id "${{ github.run_id }}" \\\n  --name lean-workspace-build \\\n  --timeout 60'},
                                              {'name': 'Download prepared Lean workspace build',
                                               'if': "steps.local-restore-lean-workspace.outcome != 'success'",
                             ...
                             'compiler-audits': [{'uses': 'actions/checkout@v6',
                                                 ...
                                                 {'name': 'Download prepared Lean workspace build',
                                                  ...
                                                  'uses': 'actions/download-artifact@v7',
                                                  'with': {'name': 'lean-workspace-build'}},
                                                 {'name': 'Restore compiler workspace build (local)',
                                                  'run': 'scripts/ci_local_persistence.sh fetch \\\n  --run-id "${{ github.run_id }}" \\\n  --name lean-workspace-compiler-build \\\n  --timeout 60'},
                                                 {'name': 'Download compiler workspace build',
                                                  'if': "steps.local-restore-compiler-workspace.outcome != 'success'",
                                                  'uses': 'actions/download-artifact@v7',
                                                  'with': {'name': 'lean-workspace-compiler-build'}},
                                                 {'name': 'Setup solc',
                             ...
                             'foundry': [{'uses': 'actions/checkout@v6',
                                          'with': {'submodules': 'recursive'}},
                                         {'name': 'Restore compiler binaries (local)',
                                          'run': 'source_dir="${VERIFY_LOCAL_ARTIFACT_ROOT}/${{ github.run_id }}/verity-compiler-binaries"\nif [ -f "$source_dir/.ready" ]; then\n  mkdir -p compiler/bin\n  cp -a "$source_dir/." compiler/bin/\n  rm -f compiler/bin/.ready\nelse\n  exit 1\nfi'},
                                         {'name': 'Download compiler binaries',
                                          'if': "steps.local-restore-compiler-binaries.outcome != 'success'",
                                          'uses': 'actions/download-artifact@v7',
 ...
 'required_makefile_check_commands': ['python3 scripts/property_pipeline.py check',
                                      ...
                                      'python3 scripts/lean_lint.py --only '
 ...
 'expected_build_commands': ['check_lean_warning_regression.py --log lake-build.log',
                             'setup_solc_slice.py',
                             'solidity_slice_mutations.py'],
 'required_build_run_commands': ['lake build PrintAxioms', 'lake build SoliditySliceSmoke'],
