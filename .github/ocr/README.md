...
- `large-lean-hotspots`: full-diff OCR does not run. The trusted router ranks diff hunks, optionally using a scout, then runs bounded semantic reviews for selected file groups. This triggers for at least 3 changed Lean files or more than 800 supported changed lines. Overall PR size does not disable this route; at most eight packets and four groups are selected. If no safe packets can be produced, no semantic packet review runs and the report explicitly says so.
...
OpenCodeReview 1.7.9 reviews full diffs for `small-lean`, `medium-lean`, and `config-docs`. The `large-lean-hotspots` route reviews selected file groups using the packet runner described below. The posted report distinguishes scout triage from semantic review, lists covered groups and failures, and states residual risk. It must not be read as full review coverage or LGTM.
