...
/-! ## Byte-level reads and the bytes-equality helper -/

/-- An aligned byte read extracts the top byte of the stored word. -/
theorem calldataByte_aligned (selector : Nat) (calldata : List Nat) (q : Nat)
    (hval : calldata.getD q 0 < Compiler.Constants.evmModulus) :
    calldataByte selector calldata (4 + 32 * q) =
      calldata.getD q 0 / 2 ^ 248 % 256 := by
  simp [calldataByte, calldataloadWord_aligned selector calldata q hval]

/-- The generated byte-for-byte equality helper is reflexive: comparing a
region against itself is `true` for every length. -/
theorem dynamicBytesEqCalldata_refl (selector : Nat) (calldata : List Nat)
    (offset length : Nat) :
    dynamicBytesEqCalldata selector calldata offset length offset length = true := by
  simp [dynamicBytesEqCalldata]
