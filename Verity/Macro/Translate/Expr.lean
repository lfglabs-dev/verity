...
def tupleExternalCallBindStmt?
  ...
  let lower (extName : String) (ext : ExternalDecl) (args : Array Term) : CommandElabM (Array Term × Array TypedLocal) := do
      ...
      let normalizations ← (resultNamesRev.reverse.toArray.zip ext.returnTys).filterMapM fun (resultName, ty) =>
        normalizeBoundValueStmt? ty rhs resultName
      pure (#[stmt] ++ normalizations, typedLocals)
  match stripParens rhs with
  | `(term| callExternal $name:ident ($[$args:term],*)) =>
      let extName := toString name.getId
  ...
  | _ =>
      ...
      | some (ext, target, args, some _, selector) =>
          ...
          let stmt ← `(Compiler.CompilationModel.Stmt.ecm
            ...
            [ $targetExpr, $[$argExprs],* ])
          let normalizations ← (resultNames.toArray.zip ext.returnTys).filterMapM fun (resultName, ty) =>
            normalizeBoundValueStmt? ty rhs resultName
