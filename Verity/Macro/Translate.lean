  ...
  | none => match elem with
      ...
      | `(doElem| let mut $name:ident : $declaredTySyntax:term := $rhs:term) =>
          ...
          unless inferredTy == declaredTy ||
              (declaredTy == .address && interfaceName?.isSome) do
      ...
      | `(doElem| $name:ident ← $rhs:term) =>
          ...
          let some localInfo := locals.find? (fun entry => entry.name == varName)
            | throwErrorAt name s!"cannot resolve type of mutable variable '{varName}'"
          -- Binding sources are not pure expressions.  In particular,
          -- `callResult` must materialize its ECM result before assigning the
          -- already-declared mutable local.
...
private partial def rewriteForEachExecutableDoElem
  ...
  | `(doElem| let mut $name:ident : $ty:term := $rhs:term) =>
      -- The executable pass must retain typed-interface aliases introduced by
      -- mutable locals as well: subsequent `prices.quote` calls rely on it.
      let varName := toString name.getId
      let interfaceName? ← interfaceNameOfTerm? params locals rhs
      let localTy ←
  ...
  | `(doElem| let mut $name:ident ← $rhs:term) =>
      -- Keep the binding in the executable program, but record the name so a
      -- following typed call or packed-storage rewrite sees the local.
      let varName := toString name.getId
      let interfaceName? ← interfaceNameOfTerm? params locals rhs
      let localTy ←
        match interfaceName? with
