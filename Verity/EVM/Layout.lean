...
Discharges `MemoryModel.Disjoint outOff (outOff + outSize) heapBase
(heapBase + heapWords)` goals automatically from the standard solc
layout invariants. Use this when proving frame conditions over
EntryPoint-shaped contracts (call buffer in scratch range, heap
allocation above the FMP+1).

Usage:

```
theorem my_frame_condition (L : SolcLayout) (S : ScratchOutputBuffer L) :
    MemoryModel.Disjoint S.outOff (S.outOff + S.outSize)
                         L.heapRegionBase (L.heapRegionBase + L.heapRegionWords) := by
  solc_disjoint L S
```

This is a macro (not a tactic-tactic) because the discharge is
parameterised by two named values. The tactic body unfolds
`Disjoint` and applies `call_buffer_disjoint_from_heap`. -/
macro "solc_disjoint" L:term S:term : tactic =>
  `(tactic| (unfold MemoryModel.Disjoint;
