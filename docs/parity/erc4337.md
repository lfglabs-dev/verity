...
| Construct | Signature | Verity | Notes |
| --- | --- | --- | --- |
| enum | `PostOpMode { opSucceeded, opReverted, postOpReverted }` | ✅ | Native uint8-backed enum declarations, members, and checked casts (#2088). |
...
| fn | `postOp(PostOpMode, bytes calldata, uint256, uint256)` | ✅ | Enum arguments erase to `uint8` in the ABI. |
