...
- No Lean axioms remain in Layer 2; 0 `sorry` placeholders remain. The `storageLookup_projectStorage` proof (previously a sorry) is now complete, using `Std.TreeMap.get?_insert` lemmas with an injectivity argument over in-range storage slots.
