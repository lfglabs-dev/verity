...
- TODO ([#2082](https://github.com/lfglabs-dev/verity/issues/2082) slice 4): typed reverts (`Stmt.requireError` / `Stmt.revertError` / `Stmt.panicCode`) and dynamic (`bytes`/`string`/array) event payloads are still `true` in `stmtTouchesUnsupportedEffectSurface`. Slice 3 covers only the static scalar block: `AbiEventObservable.scalarEmitPayload_log_observable` requires every unindexed parameter to occupy one head word, so a payload with a head/tail split is out of its domain, and the typed-revert lane still has no counterpart of that theorem even though `AbiMemoryLayout.abiEncodeArgs_memory_layout` already supplies the dynamic-block memory image both will need.
