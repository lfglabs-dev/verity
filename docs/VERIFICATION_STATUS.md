...
- TODO ([#2082](https://github.com/lfglabs-dev/verity/issues/2082)): every `Stmt.emit` is still `true` in `stmtTouchesUnsupportedEffectSurface`, and so are typed reverts (`Stmt.requireError` / `Stmt.revertError` / `Stmt.panicCode`). Slice 4 (`AbiDynamicEventObservable.dynamicEmitPayload_log_observable`) removes the *shape* restriction that slice 3 imposed — a payload with a head/tail split is now in the proved domain — but it does not yet flip that effect-surface bit, because the theorem is stated over the payload word block (`dynEmitUnindexedStores`) rather than over `CompilationModel.compileEmit`'s output. Three gaps remain, all `unsupported` today:
  ...
  - **Typed reverts**: still no counterpart of the emit theorem, even though `AbiMemoryLayout.abiEncodeArgs_memory_layout` already supplies the dynamic-block memory image that lane will need.
