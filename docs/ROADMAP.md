...
- `verity_contract` now has an opt-in automatic namespace policy for contracts that omit `storage_namespace`: `set_option verity.storageNamespace.default true` derives the stable `keccak256("{ContractName}.storage.v0")` root, offsets declared slots by that root, and records the root in `spec.storageNamespace` / `--layout-report`. Explicit `storage_namespace "key"` and `storage_namespace erc7201 "key"` override the policy, while contract-level `storage_namespace legacy` preserves raw legacy slots.
- `artifacts/storage_layout_report.json` plus `artifacts/STORAGE_LAYOUT_SUMMARY.md` now provide a committed, reviewable storage layout audit surface across `Compiler.Specs.allSpecs`, covering opt-in namespaces, declared/canonical slots, alias and reserved ranges, packed subfields, mappings, and dynamic-array fields (#1897). The Lean executable `verity-storage-layout-report` is the JSON source of truth; `scripts/generate_storage_layout_report.py` renders the Markdown summary and gates drift in `make check` (Markdown vs JSON) and `make verify-storage-layout` (JSON vs current `allSpecs`).
...
- `Compiler.Modules.Calls.selfDelegateMulticallBytes` now provides the scoped `multicall(bytes[])` surface for same-contract delegatecall loops: it checks the dynamic bytes-array offset table, copies each element payload, executes `delegatecall(gas(), address(), ptr, size, 0, 0)`, and bubbles exact revert data. Trust reports record the named ECM assumption instead of the generic proxy/upgradeability delegatecall boundary.
...
- `artifacts/layer2_boundary_catalog.json` records the theorem target, the `SupportedSpec` split, and the current temporary `SupportedBodyInterface.stmtList` helper gate.
...
- **Phase 4 (native dispatcher closure)**: `EvmYulLeanBodyClosure.lean` proves universal `compileStmtList_always_bridged` coverage for `BridgedSafeStmts`; statement-position `internalCall` / `internalCallAssign` and `externalCallBind` are now admitted when their callees resolve in an explicit `BridgedFunctionTable`, while opaque `ecm` statements still require bridgeable-output obligations before they can enter the safe-body whitelist. `EvmYulLeanSourceExprClosure.lean` proves scalar leaf closure (`compileExpr_bridgedSource_leaf`) and source-expression closure (`compileExpr_bridgedSource`) for arithmetic/comparison/bit-operation expressions, parameter length identifiers, storage, storage-array length, ADT tag/field reads, mapping reads through the abstract `mappingSlot` bridge, calldata/memory/transient reads, and syntactic `keccak256(offset, size)` emission in the `BridgedSourceExpr` fragment. `mappingSlot` remains an abstraction boundary for mapping-slot memory+keccak equivalence, while `keccak256` remains a source-semantics boundary for full end-to-end expression correctness until the source evaluator models memory-slice hashing.
...
- **Remaining to make retargeting whole-program**: connect the table-resolved `BridgedSafeStmts` call-family constructors to emitted helper/stub tables, and add bridgeable-output obligations for remaining opaque ECM modules.
