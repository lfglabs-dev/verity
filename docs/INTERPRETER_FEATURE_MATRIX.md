...
3. **Linear memory**: The IRInterpreter has single-word memory support. `mload`, `mstore`, and `returndataOptionalBoolAt` therefore remain only partially modeled in the proof interpreters today. `calldatacopy` is the exception: its destination region is modeled word-wise against the frame's calldata on both the source and IR sides, so it is fully proof-covered. `returndataCopy` now has the same word-wise region model against the frame's EIP-211 buffer on both the source and IR sides (`returndatacopyMemory`), with the zero-extent copy still the only one the admitted generic fragment exercises. Full linear memory coverage requires EVMYulLean semantic integration.
