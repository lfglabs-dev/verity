...
<CanonicalSurface>
So your port target is the `verity_contract` block; the monad/spec/proof forms in §3 are what it lowers to.
</CanonicalSurface>
...
- Verity, proof reuse: declare `verity_mixin Ownable` and
  ...
  mixin itself).
- Verity, source flatten only: `verity_contract Child is Parent where` or
  `verity_contract Child is A, B, C where` re-elaborates a copied tree.
  Parent proofs do not apply. Multi-parent is left-to-right flatten; diamonds
  are rejected. There is no C3 linearization.
...
Raw `keccak256` hashing also compiles through the typed intrinsic surface, but dynamic memory hashing still relies on an explicit primitive assumption in the current proof stack. Treat it as a separate trust boundary, archive `--trust-report`, and use `--deny-axiomatized-primitives` when the translated contract must stay inside the proved subset (see issue `#1411`).
...
> **Steps A–D are the proof layer.** They show the `StorageSlot`/`Contract`-monad `def`s, the `_spec` predicate, and the `_meets_spec` theorem that a `verity_contract` block (the canonical surface, §2) lowers to. Author in `verity_contract`; reach for these forms when you write or read the proofs.
