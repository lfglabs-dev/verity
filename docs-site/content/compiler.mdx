...
**Internal functions** via source-level `internal` helpers, `isInternal := true`, and `Stmt.internalCall`; these helpers stay out of selector dispatch and ABI JSON. **Events** via the `events` block and `Stmt.emit` (topic0 computed from the event signature); **double mappings** via `mapping2`/`setMapping2`; **uint256-keyed mappings**; **dynamic, fixed-size, and bytes array parameters** with ABI offset-based decoding.
