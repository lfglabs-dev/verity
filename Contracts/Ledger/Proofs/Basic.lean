...
theorem deposit_meets_spec (s : ContractState) (amount : Uint256) :
  ...
  refine ⟨?_, ?_, ?_⟩
  · simp [ContractResult.snd, ContractState.writeMap, beq_iff_eq, ContractState.storageMap]
  · refine ⟨?_, ?_⟩
    · intro addr h_ne
      simp [ContractResult.snd, ContractState.writeMap, beq_iff_eq, h_ne, ContractState.storageMap]
...
theorem deposit_preserves_other_balances (s : ContractState) (amount : Uint256)
  ...
  rw [deposit_unfold]
  simp [ContractResult.snd, ContractState.writeMap, h_ne, ContractState.storageMap]
...
theorem transfer_meets_spec (s : ContractState) (toAddr : Address) (amount : Uint256)
  ...
  · rw [transfer_unfold_other s toAddr amount h_balance h_eq]
    ...
    · simp [ContractState.writeMap, h_ne', ContractState.storageMap]
    · simp [ContractState.writeMap, h_ne', ContractState.storageMap]
      refine ⟨?_, ?_⟩
      · intro addr h_ne_sender h_ne_to
        simp [h_ne_sender, h_ne_to, ContractState.storageMap]
      · intro slotIdx h_slot addr
        simp [h_slot, ContractState.storageMap]
    · simp [ContractState.writeMap, Specs.sameStorageAddrContext, Specs.sameStorage, Specs.sameStorageAddr, Specs.sameStorageArray, Specs.sameContext, ContractState.storageMap,
...
theorem deposit_getBalance_correct (s : ContractState) (amount : Uint256) :
  ...
  simp [ContractResult.snd, ContractState.writeMap, Verity.bind, Bind.bind, Verity.pure, Pure.pure, Contract.run,
    getMapping, balances, ContractState.readMap, ContractState.storageMap]
