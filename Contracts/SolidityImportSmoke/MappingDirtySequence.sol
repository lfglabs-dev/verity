...
contract SequenceFixture {
    ...
    function read() external returns (uint128, bool, uint128, bytes32) {
        uint128 oldBalance = balances[msg.sender];
        bool oldAuthorization = authorized[msg.sender][address(this)];
        uint128 oldConsumed = consumed[msg.sender][bytes32(uint256(7))];
        bytes32 oldWord = words[7];
        delete balances[msg.sender];
        delete authorized[msg.sender][address(this)];
        delete consumed[msg.sender][bytes32(uint256(7))];
        delete words[7];
        return (oldBalance, oldAuthorization, oldConsumed, oldWord);
    }
}
