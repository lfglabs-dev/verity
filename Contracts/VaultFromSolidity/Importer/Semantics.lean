/-
  ...
  point as `Fn.meaning body slots env`. Nothing here inspects JSON, and nothing
  ...
  pin the write order the contract produces.
-/
...
/-- A function body either returns nothing or one `uint256`. -/
@[reducible] def Ret.denote : Ret → Type
  | .unit => Unit
  | .uint => Verity.Core.Uint256

/-- A state variable is a named `StorageSlot`, scalar or `address`-keyed. -/
...
def Expr.meaning {L : Layout} {Γ : Ctx} {t : Ty} (e : Expr L Γ t) (slots : Slots L)
    (env : Env Γ) : Verity.Contract t.denote :=
  match e with
  ...
  | .index m key =>
      Verity.bind (Expr.meaning key slots env) (fun a => Verity.getMapping (Slots.get slots m) a)
  | .arith op a b =>
      Verity.bind (Expr.meaning a slots env) (fun x =>
        Verity.bind (Expr.meaning b slots env) (fun y => checked op x y))
...
def Stmt.meaning {L : Layout} {Γ : Ctx} {r : Ret} (s : Stmt L Γ r) (slots : Slots L)
    (env : Env Γ) : Verity.Contract r.denote :=
  match s with
  | .done => Verity.pure ()
  | .ret e => Expr.meaning e slots env
  | .assign lv op rhs rest =>
      match lv with
      | .scalar sv =>
          assignWith (Verity.getStorage (Slots.get slots sv))
            (fun v => Verity.setStorage (Slots.get slots sv) v) op
            (Expr.meaning rhs slots env) (Stmt.meaning rest slots env)
      | .mapping sv key =>
          Verity.bind (Expr.meaning key slots env) (fun a =>
            assignWith (Verity.getMapping (Slots.get slots sv) a)
              (fun v => Verity.setMapping (Slots.get slots sv) a v) op
              (Expr.meaning rhs slots env) (Stmt.meaning rest slots env))
  | .local_ e rest =>
      Verity.bind (Expr.meaning e slots env)
        (fun x => Stmt.meaning rest slots (Env.cons x env))
  | .guard a b msg rest =>
      Verity.bind (Expr.meaning a slots env) (fun x =>
        Verity.bind (Expr.meaning b slots env) (fun y =>
          Verity.bind (Verity.require (Nat.ble y.val x.val) msg)
            (fun _ => Stmt.meaning rest slots env)))

/-- Every imported entry point is non-payable. -/
...
def Fn.meaning {L : Layout} {Γ : Ctx} {r : Ret} (body : Stmt L Γ r) (slots : Slots L)
    (env : Env Γ) : Verity.Contract r.denote :=
  nonpayable (Stmt.meaning body slots env)
...
attribute [solidity_import] Fn.meaning nonpayable Stmt.meaning assignWith Expr.meaning checked
  Env.get Slots.get Ty.denote Ret.denote StorageTy.denote
