...
-- likewise `Env.cons : {Γ : Ctx} -> {t : Ty} -> ...`. `mkAppOptM` is positional, so
-- the indices are supplied in that order.
...
private def importFrontend (ns : Name) (frontend : Frontend) : MetaM Unit := do
  ...
  -- handles to the AST as its `Slots` index.
  ...
  for h in registered do
    ...
    register (ns ++ `Storage ++ Name.mkSimple h.1.var) value
  ...
  for h in registered do
    if let some getter := h.1.getter then
      let value ← if h.1.mapping then
        ...
        else mkAppM ``Sol.nonpayable #[← mkAppM ``Verity.getStorage #[h.2]]
  ...
  for fn in frontend.functions do
    ...
    register (ns ++ Name.mkSimple name) value (some type)
