...
verity_contract ProxyUpgradeabilityLayoutCompatibleSmoke where
  storage
    initializedVersion : Uint256 := slot 0
    admin : Address := slot 1
    implementation : Address := slot 2
    pendingImplementation : Address := slot 3
  ...
  function stageUpgrade (nextImplementation : Address) reinitializer(initializedVersion, 2)
      local_obligations [upgrade_authorization := assumed "Caller must separately prove that only the intended admin can authorize staged upgrades.", storage_layout_compatibility := proved "The additional pendingImplementation slot only extends the existing layout without moving persisted fields."] : Unit := do
